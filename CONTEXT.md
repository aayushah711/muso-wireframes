...
- ~~**Discount stacking (owner, 2026-09-24).**~~ Removed 2026-09-25, see below. Every discount has "Can be combined with other discounts" (`d.combinable`, form checkbox `data-combinable`). POS takes several codes (`bookingDiscountCodes`, chips with ×). Summary applies the bigger of: all combinable discounts stacked (% first, then flat, each on what is left) vs the best non-combinable one alone; the rest show struck through with the reason. Seeds: bundling, Membership, Members-programs, JSW series combinable.
- ~~**"Cannot be stacked" at the till (owner, 2026-09-25).**~~ Superseded same day.
- **ONE discount per booking (owner, 2026-09-25) — all stacking removed.** No stack checkbox on the discount form, no `combinable` on seeds, stacking help rows deleted, precedence help = tie-break only. Till takes one code: a second is refused by `oneDiscountRefusal()` → "Only one discount per booking. Remove JSW001 first to use MUMKIDS150 instead." Summary applies the single best-saving candidate (typed code, automatic offers, member add-on perk); the rest show "not applied, only one discount per booking — X saves more".
- **Vouchers (owner, 2026-09-25: 1A 2A 3A).** A voucher is a discount with `kind:"voucher"` (Discounts page: "+ New voucher", or the "What is it: Discount / Voucher — free visit" switch). Form swaps the rule/amount section for "What the voucher makes free": product chips (`covers`) + optional "most free tickets per visitor type" (`caps`, empty = no limit). List/summary show a Voucher badge and "Visit free · up to 2 Adult…". Orders screen: **Discount code / Voucher** tabs over the code box (`bookingCodeTab`); a code on the wrong tab is refused. `voucherValue()` frees paid tickets of covered products up to each cap; still one discount per booking (best saving wins). **Voucher payment button removed**; a ₹0 order shows "Nothing to pay" + "Confirm free booking". Seeds: `FREETOUR01–50` (MuSo Tour, 2 Adult + 2 Child 3–11), `PRESSDAY` (Tour + Mini Golf, no cap, staff only).
...
### 20. School/Sponsored bulk import, preset persistence, dashboard panel, GST-inclusive pricing (2026-09-25)
All verified with `node --check` + DOM-stub render sweep (0 failures) + live chrome-devtools checks (the `chrome-devtools-mcp` plugin was attachable this session).

**Entry price preset now persists** — the preset `<select>` (`data-school-preset`/`data-sp-preset`) gained `data-sf="entryPreset"`/`data-spf="entryPreset"` and marks the chosen option `selected` from `d.entryPreset`; `entryPreset` was added to both capture lists so it round-trips through re-renders. Previously it filled the prices then snapped back to "— pick a preset —" on the re-render.

**Dashboard "Upcoming schools" panel (for Reception)** — `upcomingSchoolVisits()` (defined just before `const VIEWS`) collects School + Sponsored records (excludes Ended/Lost/Rejected/Cancelled), one row per future visit (or the enquiry `prefDate` when no visit yet), sorted by date, reference "today" = `DASH.todayDate`. Rendered as `schoolsPanel` (title/note, totals strip N visits/students/adults, table Date&slot · School+tag+location · Students · Adults, rows `data-open="school|sponsored:idx"`, `max-height:340px`), inserted into the dashboard return before the KPI strip.

**"Other" designation** — `SCHOOL_DESIGN` (shared by both forms) gained "Other"; selecting it reveals a conditional **Other designation** free-text field (`designationOther`, pattern mirrors "Other Board"), captured with clear-when-not-Other, re-rendered on the select's change (`[data-sf="designation"]`/`[data-spf="designation"]`), and the detail shows the typed value instead of "Other".

**Bulk CSV import — School & Sponsored** (past data; one row = one booking, one visit). Core module lives after `downloadCSV`:
- `parseCSV(text)` — RFC-4180-ish reader (quoted fields, escaped `""`, CRLF, embedded commas/newlines). Helpers `bulkSplit` (`;`/`|` → array), `bulkNum` (strips non-digits), `bulkUnderpriv` (`title:count;…` → `[{title,count}]`), `bulkStatus` (valid or default **Ended**), `bulkSlot`.
- `SCHOOL_BULK_COLS`/`SP_BULK_COLS` — full column sets (every form field; Sponsored adds OrgType/District/Sponsor/AuthNumber/NGO GST+PAN/ProofDoc/Transport/SpeciallyAbled/Underprivileged). `parseBulk(text,kind)` maps by header (case-insensitive) or positionally, validates required fields (`bulkValidateRow`: School, Location, Boards, Grade, POC, Email, Phone, VisitDate, Interested + email format) → `{records:[{line,obj,errs,ok}],okCount,errCount}`.
- `schoolRowToRecord`/`spRowToRecord` build seed-shaped records (`origin:"import"`, seeded timeline; School gets a single `payments[]` entry from `AmountPaid`; Sponsored has no payment — `invoice{raised:false}`). `bulkTemplateRows(kind)` from `BULK_SCHOOL_SAMPLES`/`BULK_SP_SAMPLES`.
- **UI**: `bulkPanel(kind)` rendered inside `school()`/`sponsored()` when `schoolBulk`/`spBulk` set (checked after the reports branch). Toolbar **Bulk upload** button (`data-bulk-open`). Flow: pick file → validation preview (✓ ready / ⚠ skipped-with-reasons) → **Import** → a **results view** (`st.done`): imported rows **with their assigned IDs**, skipped rows with reasons, **Download skipped rows** (original columns + an `Error` column, `data-bulk-skipped`), **Import another file** (`data-bulk-again`), Done. State `schoolBulk`/`spBulk` (`{fileName,result,done}`); handlers `data-bulk-open`/`-close`/`-template`/`-import`/`-skipped`/`-again` (click) + `data-bulk-file` (change, `FileReader.readAsText`). Reset on nav change and schtab/sptab switch. Panel text avoids `scrub()`-stripped phrases.

**Entry & F&B prices are GST-INCLUSIVE — system-wide (form + bulk) — UNCOMMITTED.** Anchored in `schoolCompute` (used by both modules): `gross = visitTotal + foodTotal` (inclusive); `subtotal = round(gross/1.18)` (taxable value, GST removed); `gst = gross − subtotal` (cgst/sgst split); `grand = gross` (no GST added on top). Every downstream "subtotal + gst = grand" display stays consistent automatically. Also updated: both **proforma/e-invoice** HTML+PDF — per-HSN taxable is now *extracted* (`gross/1.18`, not `amount`), tax rows read "of which: CGST/SGST 9% (incl.)", total labeled **Total (incl. GST)**, PDF "Sub total"→"Taxable value"; **detail Pricing card** labels (Visit/Food total incl. GST, Taxable value, GST 18% (incl.), Grand total incl. GST); **form hints** on entry-price + F&B fields; **bulk intro** note; **Entry-price-presets master** column headers + schema descriptions. Verified: ₹40,000 entered → total ₹40,000, GST ₹6,102 extracted (was ₹47,200 grand under old exclusive model). **Scope: School + Sponsored only** — the **Shop/retail invoice** (`shopSaleCompute`, its `inv-total` row) keeps its own exclusive/MRP tax model, deliberately untouched.

### 21. Programs validation, strike-through prices, age range (2026-09-25)
...
- Git HEAD is `31a2fff` (branch `main`, merge commit) — **in sync with `origin/main`**. Sections 12–19 plus §20's preset-persistence, dashboard "Upcoming schools" panel, "Other" designation and **bulk CSV import** are committed (`d02b994`, pushed). **Uncommitted in the working tree:** §20's **GST-inclusive pricing** change (`muso-admin-portal.html`) + this `CONTEXT.md` update — commit when ready. A merge-pull of origin's 3 commits (combinable-discounts checkout, schema alignment) auto-merged with 0 conflicts and was verified (sweep + bulk tests). Untracked docs (`VENUE-MODULE-PLAN.md`, scope docs, `.xlsx`, `package*.json`, an untracked `.gitignore`, etc.) remain untracked.
...
- Bulk-import helpers (School/Sponsored past-data CSV): `parseCSV`, `parseBulk`, `bulkValidateRow`, `schoolRowToRecord`/`spRowToRecord`, `bulkTemplateRows`, `bulkSplit`/`bulkNum`/`bulkUnderpriv`/`bulkStatus`/`bulkSlot`; `SCHOOL_BULK_COLS`/`SP_BULK_COLS`; panel `bulkPanel` + state `schoolBulk`/`spBulk`.
- **Pricing model: entry + F&B prices are GST-inclusive.** `schoolCompute` extracts GST out of the inclusive total (`grand = visitTotal + foodTotal`, `subtotal = round(grand/1.18)`, `gst = grand − subtotal`) — School + Sponsored only; the Shop invoice (`shopSaleCompute`) stays exclusive.
- Dashboard: `upcomingSchoolVisits()` powers the "Upcoming schools" panel (reference date = `DASH.todayDate`).
